...
<div class="main">
  ...
  <div class="card">
    ...
    <div class="btn-grid">
      <div></div>
      <button class="btn-fwd" onclick="motionCmd('FORWARD '+spd())">▲ Forward</button>
      <div></div>
      <button onclick="motionCmd('TURN_LEFT '+spd())">↺ Left (→270°)</button>
      <button class="btn-stop" onclick="sendCmd('STOP')">■ Stop</button>
      <button onclick="motionCmd('TURN_RIGHT '+spd())">↻ Right (→270°)</button>
      <div></div>
      <button class="btn-bwd" onclick="motionCmd('BACKWARD '+spd())">▼ Backward</button>
      <button class="btn-emg" onclick="sendCmd('S')">⚡ EMERGENCY</button>
    </div>

    <div style="display:flex;align-items:center;gap:10px;margin-top:8px;font-size:.85rem">
      <span style="color:#666">Live heading:</span>
      <span id="headingBadge" style="font-weight:bold;color:#fa4;font-size:1rem">—°</span>
      <span style="color:#444;font-size:.75rem">(straight=90° · turn target=270°)</span>
    </div>
...
// ── Motion command: auto-starts IMU stream so heading is live during moves
async function motionCmd(cmd) {{
  // Ensure Arduino is streaming IMU and SSE is open before the move
  const ms = document.getElementById('imuRate').value;
  await sendCmd('IMU_STREAM ' + ms, true);
  startSse();
  await sendCmd(cmd);
}}
...
function startSse() {{
  ...
  sse.onmessage = function(e) {{
    ...
    // Keep motor-control heading badge in sync
    document.getElementById('headingBadge').textContent = heading.toFixed(1) + '°';
    drawCompass(heading);
    pushAccel(ax, ay, az);
  }};
